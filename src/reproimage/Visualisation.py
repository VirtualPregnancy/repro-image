...
def visualise_graph_and_field(graph: nx.graph, coords: np.array, field, field_name='radii', title='', need_remap = True, radius_scale_factor = 10, show=True):
    if title == '':
    ...
    pod_tube = pod.tube(scalars=field_name, radius=1.0, radius_factor = radius_scale_factor)
...
def visualise_segmentation(seg: np.array, opacity_setting = 0.5):
    ...
    if seg.max() == 0:
        ...
        verts, faces, normals, values = sk.measure.marching_cubes(seg, 0)
        ...
        pl.add_mesh(mesh, opacity=opacity_setting)
        pl.show()